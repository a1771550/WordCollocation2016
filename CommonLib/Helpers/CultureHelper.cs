...
namespace CommonLib.Helpers
{
	public static class CultureHelper
	{
		...
		private static readonly List<string> _cultuers = new List<string> {"zh-Hant","ja-JP", "zh-Hans"};
		...
		/// <summary>
		/// Returns a valid culture name based on "name" parameter. If "name" is not valid, it returns the defalut culture "en-US"
		/// </summary>
		...
		public static string GetImplementedCulture(string name)
		{
			...
			if (_cultuers.Count(c => c.Equals(name, StringComparison.InvariantCultureIgnoreCase)) > 0) return name;

			// Find a close match. For example, if you have "en-US" defined and the user requests "en-GB", 
			// the function will return closes match that is "en-US" because at least the language is the same (ie English)  
			string n = GetNeutralCulture(name);
			foreach(var c in _cultuers)
		...
		/// <summary>
		/// Returns default culture name which is the first name declared (eg: en-US)
		/// </summary>
		/// <returns></returns>
